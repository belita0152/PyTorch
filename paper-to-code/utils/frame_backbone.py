...
class FrameBackBone(nn.Module):
    def __init__(self, fs: int, time_window: int, second: int, time_step: float, out_dim: int = 512):
        super().__init__()
        self.fs = fs
        self.time_window = time_window
        self.second = second
        self.time_step = time_step
        self.model = BackBone(input_size=fs * time_window, input_channel=1, layers=[1, 1, 1, 1])
        self.feature_num = self.model.get_final_length() // 2
        self.feature_layer = nn.Sequential(
            nn.Linear(self.model.get_final_length(), self.feature_num),
            nn.ELU(),
            nn.Linear(self.feature_num, out_dim)
        )
...
if __name__ == '__main__':
    ...
    # )
    fb = FrameBackBone(fs=100, second=30, time_step=3, time_window=3)
    ss = fb(
        torch.randn(50, 3000)
    )
    print(ss.shape)
